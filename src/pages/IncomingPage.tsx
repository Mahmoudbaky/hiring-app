...
import { Btn, Th, Td, AttachIcon, DLabel, DSelect, Pagination } from '@/components/shell';
        ...
        <CardHeader className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <CardDescription>مراجعة وتصنيف المتقدمين للوظائف الشاغرة</CardDescription>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <Badge variant="neutral" className="h-7 px-3">
              <span className="tabular font-semibold">{filtered.length}</span>
              <span className="text-muted-foreground">طلب إجمالي</span>
            </Badge>
            <Btn
              variant="outline" size="sm"
              onClick={() => toast.success(`تم تصدير ${filtered.length} طلب إلى Excel`)}
            >
            ...
            <div className="relative">
              ...
              {filterOpen && (
                <div className="absolute top-full mt-2 end-0 w-72 bg-card border rounded-md shadow-md p-4 anim-pop z-20 space-y-3">
                  <div>
              ...
              {view.map((a) => (
                <tr key={a.id} className="row">
                  ...
                  <Td>
                    ...
                    <div className="text-[11.5px] text-primary mt-0.5">{a.jobMeta}</div>
                  </Td>
                  <Td>
